...
export interface JevRouteResult {
  ...
  picked: string
  reason: string
  /** 场景名单修正（与当前一致时 undefined，不落 presence 行）。各层 since 由 setScene 继承。 */
...
export async function jevRoute(input: JevRouteInput): Promise<JevRouteResult | undefined> {
  ...
  for (const n of input.allNames) {
    questions[`present_${n}`] = {
      type: 'noul',
      instructions: `根据对话判断：${n} 此刻是否在当前场景现场（人在场景里）。两个方向的判定标准不同：
...
对话是最新事实：记录落后于剧情时以对话为准，更新记录正是你的职责。`,
  ...
  try {
    ...
    input.log?.({
      ...
      ...(sceneUnchanged ? { scene: '未变' } : { scene: { present, remote: remote.map(l => `${l.character}(${l.perceive})`), overhear: overhear.map(l => `${l.character}(${l.perceive})`) } }),
      knows: [...knows],
      told: [...told],
      stateDirty,
      elapsedMs: Date.now() - t0,
...
export async function jevAfterReply(input: {
  ...
  questions['next_speaker'] = {
    type: 'choice',
    instructions: '选出下一位**输出内容**的角色或用户：角色的输出可以是说话，也可以是动作、神态，或沉默（沉默本身就是一种回应）。刚输出过内容的角色通常不立即连续输出；但刚被哀求、被点名、被质问、被逼迫回应的角色必须获得输出权——哪怕他会保持沉默，他的沉默就是对哀求的回应。判断刚输出内容的一方在跟谁说话：括号里的动作描写（如"看着X说道""转向X"）通常标明真正的对象，台词中出现的名字可能只是被谈论的第三者，冲突时以括号指向为准。只有当剧情的下一步明显该由用户决定、或对话已自然告一段落时才选用户。不确定时选用户。',
    criteria,
  }
  try {
    const t0 = Date.now()
...
export async function askSceneSummarizer(input: {
  ...
  const prompt = [
    ...
    '- 不回顾历史，不解释来龙去脉，不推断是谁做了什么、怎么发生的（可见证据可客观陈述，如"颈部有勒痕"）。',
