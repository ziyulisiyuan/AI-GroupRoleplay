...
try {
  // ── 1) 坏行忽略 + id 续号（store 级）
  {
    ...
    writeFileSync(join(dir, '剧情.jsonl'), [
      ...
      '{"type":"msg","id":1,"role":"user","name":"你","text":"一","round":1,"visible_to":"all","ts":"t"}',
      '{"type":"msg","id":2,"role":"user","name":"你","text":"二","round":2,"visible_to":"all","ts":"t"}',
      '{"type":"msg","id":3,"role":"user","name":"你","text":"坏行——故意截断', // 尾部半行
      '{"type":"msg","id":4,"role":"user","name":"你","text":"四","round":4,"visible_to":"all","ts":"t"}',
    ].join('\n') + '\n', 'utf8')
  ...
  // ── 2) 按 text 撤回：重启 + 重放都不复活
  {
    ...
    assert.equal(replay.memory.filter(e => e.text.includes('测试内容一')).length, 0, 'rebuild 重放后撤回条目不得复活')
  }
  ...
  // ── 3) edit 活账本
  {
    ...
    s.editMessage(1, '（测试内容一改）')
    assert.ok(!s.memoryOf('角色甲').some(e => e.text.includes('测试内容一')), '被撤回的 mid 不得因 edit 复活')
    // 再落一条可见消息并回填，然后 edit：条目文本必须跟着新文本走
    const store = StoryStore.open(accDir, accName)
    store.append('user', '你', '（测试内容二）', ['角色甲'])
    const s3 = GroupSession.open(accName)
    const before = s3.memoryOf('角色甲')
    const entry = before.find(e => e.text.includes('测试内容二'))
    assert.ok(entry !== undefined, '新消息应已回填')
    s3.editMessage(entry.mid!, '（测试内容四）')
    const after = s3.memoryOf('角色甲')
    assert.ok(!after.some(e => e.text.includes('测试内容二')), '旧文本不得残留在记忆里（活账本）')
    assert.ok(after.some(e => e.text.includes('测试内容四')), '记忆条目必须同步改写为新文本')
    ...
    assert.ok(!replay2.memory.some(e => e.text.includes('测试内容二')), '重放后同样只有新文本')
    assert.ok(replay2.memory.some(e => e.text.includes('测试内容四')), '重放后新文本在账')
    assert.ok(!readFileSync(join(accDir, '剧情.jsonl'), 'utf8').includes('测试内容二'), '手改后旧原文不得残留在日志里（含账本行）')
  }
  ...
  // ── 4b) 删除消息 = 记忆一并撤回（所有引用该消息的账本条目消失且不复活）+ 日志物理移除
  {
    const s = GroupSession.open(accName)
    const store = StoryStore.open(accDir, accName)
    store.append('user', '你', '（测试内容三）', ['角色甲', '角色乙'])
    const s2 = GroupSession.open(accName)
    assert.ok(s2.memoryOf('角色甲').some(e => e.text.includes('测试内容三')), '甲应已登记该消息')
    assert.ok(s2.memoryOf('角色乙').some(e => e.text.includes('测试内容三')), '乙应已登记该消息')
    const delId = store.messages.at(-1)!.id
    s2.deleteMessage(delId)
    assert.ok(!readFileSync(join(accDir, '剧情.jsonl'), 'utf8').includes('测试内容三'), '删除 = 消息行物理移除（原始日志不保留原文）')
    assert.equal(StoryStore.open(accDir, accName).messages.some(m => m.id === delId), false, '重启后日志里也没有该消息')
    const s3 = GroupSession.open(accName)
    assert.ok(!s3.memoryOf('角色甲').some(e => e.text.includes('测试内容三')), '删除后甲的记忆必须同步清除')
    assert.ok(!s3.memoryOf('角色乙').some(e => e.text.includes('测试内容三')), '删除后乙的记忆必须同步清除')
    assert.ok(!s3.snapshot().messages.some(m => m.text.includes('测试内容三')), '删除的消息从可见视图消失')
